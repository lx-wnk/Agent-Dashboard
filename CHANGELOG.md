...
- An ACP spawner's executable was accepted without the trust check every other spawner command gets. The `command` column on a spawner row is validated on create and update against the trusted-command allow-list (bare names like `claude`/`npx`, or absolute paths under a trusted bin directory), but an ACP spawner keeps its actual executable in `adapter_config.command` instead, which only checked the key name and a 4096-character length cap — an admin session pointing an ACP spawner at an arbitrary binary under `/tmp` was accepted without rejection, even though the route is already admin-gated and RCE-equivalent by design. `ValidateAdapterConfig` now runs any adapter_config key that holds a command value (currently only `acp`'s `command`) through the same `services.ValidateSpawnerCommand` check, keyed off a small per-adapter-type table so a future adapter gains the same coverage automatically. An empty `command` is still accepted — it is optional for ACP and fails with a clear message at spawn time instead — and existing rows are unaffected since the check runs only at the write boundary.
