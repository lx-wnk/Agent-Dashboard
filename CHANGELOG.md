...
- **Group by spawner** on the dashboard roster — buckets agents by the spawner of the pipeline task they were spawned for; agents without a task collect in a trailing **No spawner** group. The option is hidden while the spawner filter narrows the roster to a single spawner (the grouping would be redundant) and returns once the filter is cleared back to **All spawners**.
