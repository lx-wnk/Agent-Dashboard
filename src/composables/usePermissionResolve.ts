...
export function usePermissionResolve() {
  ...
  async function resolveAgent(agent: Agent, outcome: 'granted' | 'denied'): Promise<string | null> {
  ...
  async function approveAll(agents: Agent[], outcome: 'granted' | 'denied' = 'granted'): Promise<string | null> {
    ...
    await Promise.all(
      Array.from(byTask.entries()).map(async ([taskId, ids]) => {
        try {
          await bulkResolvePermissionRequests(taskId, ids, outcome)
        }
        catch (err) {
