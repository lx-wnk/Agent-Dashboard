...
describe('usePipelineConfig singleton', () => {
  ...
  it('fires fetch only once across multiple calls and shares state', async () => {
    ...
    const { usePipelineConfig } = await import('@/features/pipeline/composables/usePipelineConfig')
  ...
  it('retries fetch after failure', async () => {
    ...
    const { usePipelineConfig: usePipelineConfig1 } = await import('@/features/pipeline/composables/usePipelineConfig')
    usePipelineConfig1()
    await new Promise(r => setTimeout(r, 0))
    ...
    const { usePipelineConfig: usePipelineConfig2 } = await import('@/features/pipeline/composables/usePipelineConfig')
