...
Agents spawned from the dashboard run as interactive **live** sessions rather than one-shot runs, so you can converse with them as they work. Live injection uses tmux when it's available (the agent runs in a detached session) and falls back to a built-in pty broker otherwise. The spawned agent's chat modal opens automatically once it appears on the roster. When a live session asks an interactive multiple-choice question (`AskUserQuestion`), it surfaces in the **Needs you** triage band as "Needs answer" with the options inline — pick them and **Send answer** drives the session's terminal selector with real keystrokes (it requires a tmux-backed session; sessions that can't be driven live show the question read-only with a hint to answer in the terminal).
