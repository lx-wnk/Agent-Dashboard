...
| Obsidian vault (Local REST API, once configured) | Note content, read over HTTPS from the vault's Local REST API plugin | Indexing notes into the memory store as pointers (`memory.write`); on-demand read/search/write/delete by an agent holding the matching `obsidian.*` capability |
