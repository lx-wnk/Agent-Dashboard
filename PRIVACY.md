...
| `ps eww` (macOS) / `/proc/<pid>/environ` (Linux) | The environment block of monitored agent processes. Only `CLAUDE_CONFIG_DIR` is extracted — nothing else is retained, logged, persisted, or transmitted. The whole block is read transiently because neither OS offers a single-variable read. | Attributing a session to its configured spawner profile |
