...
- [ ] **Step 2: Docs.**
  ...
  - `layer2-project-core.md` SSOT rows: hub geometry constants `src/features/hub/hubGeometry.ts`; camera thresholds `src/features/hub/hubCamera.ts`; graph response type (client, hand-kept parity with `server/internal/api/obsidian/handler.go`) `src/features/hub/graphApi.ts`; the shared 429 retry `src/utils/fetchWithRateLimitRetry.ts` (`fetchWithRateLimitRetry`, used by `useWorkspace` and `useObsidianGraph`).
