...
- Produces: `POST /api/applications` with body `{name, command, args, env}` → 201 `applicationView`; the view gains `entry` (`{type, command, args, env}`, secrets never in it) and `exportToClaude`; `PATCH` accepts `command`, `args`, `env` (as one `entry` object) alongside today's `attachAll`/`requiredEnv`.
