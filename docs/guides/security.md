...
The `agent-dashboard grants` CLI is the only user-facing grant-management surface
...
the SQLite database directly, the same way `agent-dashboard settings` and
`agent-dashboard plugins` do, so it also works while the server is down.
