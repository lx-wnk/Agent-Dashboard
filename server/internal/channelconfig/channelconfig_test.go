...
func TestWriteTempConfig_JSONContract(t *testing.T) {
	...
	entry, ok := cfg.MCPServers[channelconfig.ChannelServerName]
	require.True(t, ok, "mcpServers must contain kontor-channel key")
	require.Equal(t, binaryPath, entry.Command,
		"the channel entry's command must equal the provided binary path")
}
...
func TestConfigJSON_ContainsChannelArg(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(s), &cfg))
	entry := cfg.MCPServers[channelconfig.ChannelServerName]
...
func TestConfigJSON_NilTaskAPIIsUnchanged(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t,
		`{"mcpServers":{"kontor-channel":{"command":"/bin/agent-dashboard","args":["channel"]}}}`,
		got)
}
...
func TestConfigJSON_TaskAPIAddsASecondServer(t *testing.T) {
	...
	tasks := parsed.MCPServers[mcp.ServerName]
	...
	require.Equal(t, "/bin/agent-dashboard", parsed.MCPServers[channelconfig.ChannelServerName].Command)
}
