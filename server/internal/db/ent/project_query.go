...
func (_q *ProjectQuery) QueryFolders() *ProjectFolderQuery {
	query := (&ProjectFolderClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) WithFolders(opts ...func(*ProjectFolderQuery)) *ProjectQuery {
	query := (&ProjectFolderClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) loadFolders(ctx context.Context, query *ProjectFolderQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectFolder)) error {
	...
	query.withFKs = true
	query.Where(predicate.ProjectFolder(func(s *sql.Selector) {
	...
	for _, n := range neighbors {
		fk := n.project_folders
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_folders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
