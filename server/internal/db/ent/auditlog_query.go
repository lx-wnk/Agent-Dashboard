...
func (_q *AuditLogQuery) QueryTask() *TaskQuery {
...
func (_q *AuditLogQuery) WithTask(opts ...func(*TaskQuery)) *AuditLogQuery {
	query := (&TaskClient{config: _q.config}).Query()
...
func (_q *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if query := _q.withTask; query != nil {
		if err := _q.loadTask(ctx, query, nodes, nil,
			func(n *AuditLog, e *Task) { n.Edges.Task = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuditLogQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Task)) error {
...
func (_q *AuditLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTask != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldTaskID)
		}
	}
