...
func TestSpawnerRepo_CreateNormalizesNilArgsAndEnv(t *testing.T) {
	...
	s, err := r.Create(t.Context(), "X", "x-nil", "claude", nil, nil, nil, nil, false)
	require.NoError(t, err)
...
func TestSpawnerRepo_SlugUniqueness(t *testing.T) {
	...
	_, err := r.Create(t.Context(), "A", "dup-slug", "claude", nil, nil, nil, nil, false)
	require.NoError(t, err)

	_, err = r.Create(t.Context(), "B", "dup-slug", "claude", nil, nil, nil, nil, false)
	require.Error(t, err)
}
...
func TestSpawnerRepo_List(t *testing.T) {
	...
	_, err := r.Create(t.Context(), "A", "list-a", "claude", nil, nil, nil, nil, false)
	require.NoError(t, err)
	_, err = r.Create(t.Context(), "B", "list-b", "claude", nil, nil, nil, nil, false)
	require.NoError(t, err)
...
func TestSpawnerRepo_Update(t *testing.T) {
	...
	s, err := r.Create(t.Context(), "Old", "upd", "claude", []string{"a"}, map[string]string{"K": "V"}, nil, nil, false)
	require.NoError(t, err)
	...
	cleared, err := r.Update(t.Context(), s.ID, nil, nil, nil, nil, nil, nil, nil, false, true)
	require.NoError(t, err)
	require.Nil(t, cleared.Description)

	model := "opus"
	withModel, err := r.Update(t.Context(), s.ID, nil, nil, nil, nil, nil, &model, nil, false, false)
	require.NoError(t, err)
	require.NotNil(t, withModel.ModelOverride)
	require.Equal(t, "opus", *withModel.ModelOverride)

	noModel, err := r.Update(t.Context(), s.ID, nil, nil, nil, nil, nil, nil, nil, true, false)
	require.NoError(t, err)
...
func TestSpawnerRepo_DeleteBuiltInRejected(t *testing.T) {
	...
	s, err := r.Create(t.Context(), "BI", "built-in", "claude", nil, nil, nil, nil, true)
	require.NoError(t, err)
...
func TestSpawnerRepo_DeleteInUseByTask(t *testing.T) {
	...
	s, err := r.Create(t.Context(), "T", "in-use-task", "claude", nil, nil, nil, nil, false)
	require.NoError(t, err)
...
func TestSpawnerRepo_DeleteInUseByProject(t *testing.T) {
	...
	s, err := r.Create(t.Context(), "P", "in-use-project", "claude", nil, nil, nil, nil, false)
	require.NoError(t, err)
...
func TestSpawnerRepo_DeleteHappyPath(t *testing.T) {
	...
	s, err := r.Create(t.Context(), "Dispose", "dispose", "claude", nil, nil, nil, nil, false)
	require.NoError(t, err)
